...
umap_plot_clusters_scaled <- ggplot(umap_data_clusters_scaled, aes(x = V1, y = V2, color = Cluster)) +
  geom_point() +
  labs(title = paste("UMAP - Seurat Clusters (Scaled) - Algorithm:", best_algorithm_scaled, ", Resolution:", best_resolution_scaled)) +
...
umap_plot_population_scaled <- ggplot(umap_data_population_scaled, aes(x = V1, y = V2, color = pops_sub)) +
  geom_point() +
  labs(title = paste("UMAP - Populations (Scaled) - Algorithm:", best_algorithm_scaled, ", Resolution:", best_resolution_scaled)) +
  guides(color = guide_legend(override.aes = list(size = 4))) +
  theme_minimal()
